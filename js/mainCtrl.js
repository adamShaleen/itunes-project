angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  //*remember, that method should be expecting an artist name. The artist name is
  //coming from the input box on index.html, head over there and check if that input
  //box is tied to any specific model we could use. ng-model='artist'.
  //Also note that that method should be returning a promise, so you could use .then in
  //this function.
...
// I changed the grid data above instead of doing the looping directions below.
  ...
  //fafield on your web app, and then console.log the result

  //If everything worked you should see a huge array of objects inside your console.
  //That's great! But unfortunately that's not what ng-grid is expecting. What you need
  //to do now is sort the data you got back to be an object in the following format.
    /*
